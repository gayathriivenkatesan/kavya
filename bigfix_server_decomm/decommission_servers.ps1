...
foreach($item in $decomm_servers){
      ...
      if($data)
      {
          ...
          $status = Invoke-RestMethod -Method Delete -Credential $cred -Headers $headers -Uri $url
          if($status -eq "ok"){
...
function Email($path1)
{
    ...
    $to = @("madugula.shivani@blueyonder.com","sukanyadevi.velaga@blueyonder.com","rama.bk@blueyonder.com")
    #,"CS-India-Bladelogic@blueyonder.com","abhishek.kona@blueyonder.com")
    $cc = @("madugula.shivani@blueyonder.com","abhishek.kona@blueyonder.com")
