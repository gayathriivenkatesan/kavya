...
if ($domain.name -eq $null){
	...
	if($s){
		$s3=get-aduser -Identity $usernames -Properties passwordlastset
		$s1=($s3.passwordlastset).AddHours(24)
		$s2=get-date 
		if($s2 -lt $s1){
		Write-host "Failed - Try reset password after $s1 or Try to provide password thats meeet the length,complexity  or history  requirement of the domain"
	    }
        #else{
            #$s3=get-aduser -Identity $username -Properties *|select-object  Lockedout
            #if($s3.Lockedout.tostring()=='True'){
