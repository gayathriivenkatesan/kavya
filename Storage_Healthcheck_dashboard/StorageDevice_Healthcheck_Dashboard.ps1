...
if ($finalresult5.Status -eq "Online") {
 ...
 Write-Host "Checking for hashow"
 Write-Host "########################################"
...
$result6 = $true
if($data.trim() -ne "hashow: Not supported on this platform"){
    Write-Host "$data"
    $phrases = @(
        "Active",
        "Standby",
    ...
    foreach ($phrase in $phrases) {
        ...
        if (-not ($data -match "\b" + [regex]::Escape($phrase) + "\b")) {
            $result6 = $false
 ...
 if($result1.Status  -eq 'Healthy' -and $result2.Status -eq 'Healthy' -and $result3.Status -like 'Healthy' -and $result4.Status -eq 'Healthy' -and $result5 -eq 'Healthy' -and $result6 -eq $true )
   {
    $finalresult='Healthy'
   }
   else
...
$htmlContent += "</table>"
    ...
    $CcEmail = 'CS-Automation-Team@blueyonder.com', 'suresha.us@blueyonder.com'
    #$CcEmail = 'Deshikasri.C@blueyonder.com', 'Avil.S@blueyonder.com'
