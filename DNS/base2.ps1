...
foreach($i in $a){
    ...
    $pp=Get-DnsServerResourceRecord -ZoneName $domainname -ComputerName  $comp -Name $hostname -ErrorAction Ignore|where {$_.RecordData.IPv4Address.IPAddressToString -eq $dns_host_Ip} -ErrorAction SilentlyContinue
    #Displaying the hostname in Reverse look up Zones

    $mix=$dns_host_Ip.Split(".")

    $domain2=$mix[1]+"."+$mix[0]+"."+"in-addr.arpa"
    $domain3=$mix[2]+'.'+$mix[1]+"."+$mix[0]+"."+"in-addr.arpa"
    $host2=$mix[3]+"."+$mix[2]
    $host3=$mix[3]

    $dns2=Get-DnsServerResourceRecord -ZoneName $domain2  -ComputerName  $comp  -Name $host2 -ErrorAction SilentlyContinue
    if($dns2 -eq $null){
     $dns2=Get-DnsServerResourceRecord -ZoneName $domain3  -ComputerName  $comp  -Name $host3 -ErrorAction SilentlyContinue
     if($dns2 -ne $null){
     $domain1=$domain3
     $host1=$host3
     }
...
############# REMOVING DNS SERVER FROM DNS #########
  try{
      if($pp){
        $dns=Get-DnsServerResourceRecord -ZoneName $domainname -ComputerName  $comp -Name $hostname -ErrorAction Ignore|where {$_.RecordData.IPv4Address.IPAddressToString -eq $dns_host_Ip} -ErrorAction SilentlyContinue
        $b1=@()
        ...
        if($dns -ne $null){
            ...
            Remove-DnsServerResourceRecord -ZoneName $domainname -ComputerName $comp -RRType $dns.RecordType -Name $hostname -Force  -Confirm:$false -ErrorAction SilentlyContinue
            ...
            if($dns1 -eq $null){
               ...
               $b="Forwardzone- $hostname is removed from $domainname "
               $b1=$b
            }
            else{
               write-host "Failed to remove $hostname from $domainname" -ForegroundColor Red
               $b="Forwardzone- Failed to remove $hostname from $domainname "
               $b1=$b
            }
        }
               ...
               $b="Forwardzone- Note: $hostname not found in DNS server"
               $b1=$b
        }
        #Removing Hostname in Reverse Lookup Zones 
        if($dns2 -ne $null){
            #$dns4=Get-DnsServerResourceRecord -ZoneName $domain1  -ComputerName  $comp  -Name $host1 -ErrorAction SilentlyContinue
            ...
            Remove-DnsServerResourceRecord -ZoneName $domain1 -ComputerName $comp -RRType $dns2.RecordType -Name $host1 -Force  -Confirm:$false -ErrorAction SilentlyContinue 
            ...
            if($dns3 -eq $null){
               ...
               $b="$b1,ReverseZone- $host1 is removed from $domain1"
               
            }
            else{
               write-host "Failed to remove $host1 from $domain1" -ForegroundColor Red
               $b="$b1,ReverseZone- Failed to remove $host1 from $domain1"
            }
        }

        else{
               Write-host "$hostname not found in DNS server" -ForegroundColor Red
               $b="$b1,ReverseZone-Note: $hostname not found in DNS server"
        }
    }
    else{
        #Removing Hostname in Reverse Lookup Zones 
        if($dns2 -ne $null){
            ...
            if($dns3 -eq $null){
               write-host "Post-check $host1 is removed from $domain1" -Foregroundcolor Cyan
               $b="Forwardzone- Note: $hostname not found.Reversezone- $host1 is removed from $domain1"
               
            }
            else{
               write-host "Failed to remove $host1 from $domain1" -ForegroundColor Red
               $b="Forwardzone- Note: $hostname not found.Reversezone- Failed to remove $host1 from $domain1"
            }
        }

        else{
               Write-host "$hostname not found in DNS server" -ForegroundColor Red
               $b="Failed Record $hostname not found in Forward and Reverse lookupzone"
        }
