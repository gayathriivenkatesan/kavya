...
if (Test-Connection -ComputerName 'dljdaprdsw1ha.jdadelivers.com' -Count 2 -Quiet) {
        $ipamServer='dljdaprdsw1ha.jdadelivers.com'
        # Exit the loop when a pingable server is found
}
elseif(Test-Connection -ComputerName 'dljdaprdswa1v.jdadelivers.com' -Count 2 -Quiet){
    $ipamServer='dljdaprdswa1v.jdadelivers.com'
} 
elseif(Test-Connection -ComputerName 'https://dljdaprdswa2v.jdadelivers.com/' -Count 2 -Quiet){
     $ipamServer='dljdaprdswa2v.jdadelivers.com'
}
else {
      $ipamServer=''
 }
...
try{
	foreach($i in $a)
	{
		...
		#$swis=Connect-Swis -host $ipamServer -username $username -password $res
...
$g1="<html>
    ...
    table, th, td {
     ...
     <h4 ><b>Executed on: <span >$ipamServer</span></b></h4>
     <table>
     <thead>
     <tr>
	 <th>SFCase</th>
